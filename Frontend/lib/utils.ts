...
export const getAppConfig = cache(async (headers: Headers): Promise<AppConfig> => {
  if (CONFIG_ENDPOINT) {
    ...
    try {
      // ✅ For local development, return defaults if no sandbox ID
      if (!sandboxId) {
        console.log('🏠 No sandbox ID - using default config for local development');
      ...
      if (response.ok) {
        ...
        // ✅ Return defaults on fetch failure
        return APP_CONFIG_DEFAULTS;
      }
    } catch (error) {
  ...
  // If no CONFIG_ENDPOINT, use defaults
  return APP_CONFIG_DEFAULTS;
});
